...
const skill = _.uniq(faker.lorem.words(5).split(' '));
...
describe('G0ver Mutation', () => {
  ...
  it('update skill of one g0ver', async () => {
    const sample = _.sample(data);

    query.mockClear();
    query.mockReturnValueOnce(Promise.resolve([{ id: sample.id, username: sample.username }]));
    const result = await graphql(Schema, request({ username: sample.username, skill: skill }));
    expect(result.errors).toBeUndefined();
    expect(sqlparser(query, 0)).toEqual({ username: sample.username, limit: 1, table: 'g0ver', method: 'select' });
    expect(sqlparser(query, 1)).toEqual({
      id: sample.id,
      username: sample.username,
      skill: JSON.stringify(_.uniq(sample.skill.concat(skill))),
      updated_at: true,
      table: 'g0ver',
      method: 'update',
    });
